...
export default function Clouds({ hour, isRaining }: CloudsProps) {
  ...
  // --- CORES ORIGINAIS DO NEONWAVE COM ADAPTAÇÃO PARA CHUVA --- //
  const currentColors = useMemo(() => {
    if (isRaining) {
      return {
        core: "#1f2638",
        mist: "rgba(20, 25, 40, 0.6)",
      };
    }
    return {
      core: isNight ? "#3a4466" : "#ffffff",
      mist: isNight ? "rgba(40, 45, 70, 0.7)" : "rgba(255, 255, 255, 0.4)",
    };
  }, [isRaining, isNight]);

  // O seu gerador de nuvens original do NeonWave
  const randomClouds = useMemo<CloudConfig[]>(() => {
  ...
  return (
    ...
    >
      <style>{`
        ...
        /* O segredo do NeonWave: Várias camadas de sombra concentradas */
        .cloud-core {
          position: relative;
          background: ${currentColors.core};
          border-radius: 100px;
          filter: blur(8px); /* Blur original que você curte */
          transition: background 5s ease-in-out, box-shadow 5s ease-in-out;
          box-shadow: 
            inset -15px -15px 30px rgba(0,0,0,0.05),
            20px -15px 40px ${currentColors.mist},
            -20px -10px 30px ${currentColors.mist},
            0px 10px 30px ${currentColors.mist};
        }
